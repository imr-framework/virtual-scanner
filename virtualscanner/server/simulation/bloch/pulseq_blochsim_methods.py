...
from virtualscanner.server.simulation.bloch.util import *
from math import pi
...
# TODO  incorporate B0, B1 maps
def sim_single_spingroup_v2(loc_ind,phantom,seq_info,scanner_info,sg_type='Default',b=0):
    """Function for applying a seq on a spin group and retrieving the signal
    ...
    if sg_type == 'Default':
        isc = sg.SpinGroup(loc=sgloc,pdt1t2=phantom.get_params(loc_ind),df=b0df)
    elif sg_type == 'Solver':
        isc = sg.NumSolverSpinGroup(loc=sgloc, pdt1t2=phantom.get_params(loc_ind),df=b0df)
    elif sg_type == 'Diffusion':
        isc = sg.SpinGroupDiffusion(loc=sgloc, pdt1t2=phantom.get_params(loc_ind),df=freq_offset,
                                    D=phantom.get_diffusion_coeff(loc_ind),b=b)
