...
def main(dicom_map_path: str, intensity_range_lb: np, intensity_range_ub: np, map_size: np):  # lb: lower bound, ub: upper bound
    ...
    angle_template_vec = np.array([[sphere_all_loc_template[0, 0]-rot_center[0, 0], sphere_all_loc_template[0, 1]-rot_center[0, 1]]])
    ...
    for n3 in range(image_size[2]):  # process all maps in the folder
        map_data_final_gray = np.matrix.copy(map_data_final[:, :, n3])  # set up a new variable used for image processing and circle detection
        ...
        circles_all[:, 2] = np.squeeze(0.8*np.matlib.repmat(np.amin(circles[0, :, 2]), circles.shape[1], 1))  # use smallest radius for calculating means
        for n4 in range(circles.shape[1]):  # loop through all the circles (may not be same number as num_spheres due to false positives)
            ...
            sphere_1_loc_map[0, :, n3] = circles_all[np.argmax(circle_mean)]  # find the loc and radius of sphere 1 for all maps

        angle_map_vec = np.array([[sphere_1_loc_map[0, 0, n3]-rot_center[0, 0], sphere_1_loc_map[0, 1, n3]-rot_center[0, 1]]])
        rot_angle = np.arccos(np.divide(np.dot(np.squeeze(angle_template_vec), np.squeeze(angle_map_vec)), np.linalg.norm(angle_template_vec)*np.linalg.norm(angle_map_vec)))  # calcualte rot angle
    ...
    for n5 in range(num_spheres):
        ...
        circle_plot = plt.Circle((sphere_all_loc_map[n5, 0, 0], sphere_all_loc_map[n5, 1, 0]), sphere_all_loc_map[n5, 2, 0], color='r', fill=False)
