...
if __name__ == '__main__':
    ...
    myseq = psl.make_pulseq_irse(fov,n,thk,fa,tr,te,ti,enc='xyz',loc=0,write=False)

    loc_ind_list = myphantom.get_list_inds()
    pool = mp.Pool(mp.cpu_count())


    results = pool.starmap_async(sim_single_spingroup, [(loc_ind, df, myphantom, myseq) for loc_ind in loc_ind_list]).get()
    pool.close()
